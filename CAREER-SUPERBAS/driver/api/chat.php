 ...
 *   POST ?action=blast             → send message to multiple drv_candidates
 */
...
if ($action === 'send' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($replyToId > 0) {
        $db = getDB();
        $rp = $db->prepare("SELECT sender_name, message, message_type FROM drv_chat_messages WHERE id = ? AND candidate_id = ?");
    ...
    $db = $db ?? getDB();
    try {
        $stmt = $db->prepare("INSERT INTO drv_chat_messages 
...
if ($action === 'history') {
    ...
    if ($beforeId > 0) {
        $stmt = $db->prepare(
            "SELECT * FROM drv_chat_messages WHERE candidate_id = ? AND id < ? ORDER BY id DESC LIMIT ?"
        );
        $stmt->execute([$candidateId, $beforeId, $limit]);
    } else {
        $stmt = $db->prepare(
            "SELECT * FROM drv_chat_messages WHERE candidate_id = ? ORDER BY id DESC LIMIT ?"
        );
...
if ($action === 'mark_read' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $otherType = $sender['type'] === 'user' ? 'admin' : 'user';
    $db->prepare(
        "UPDATE drv_chat_messages SET is_read = 1 WHERE candidate_id = ? AND sender_type = ? AND is_read = 0"
...
if ($action === 'conversations') {
    ...
    $sql = "SELECT 
                ...
                (SELECT message FROM drv_chat_messages WHERE id = MAX(cm.id)) as last_message,
                (SELECT sender_type FROM drv_chat_messages WHERE id = MAX(cm.id)) as last_sender_type,
                (SELECT message_type FROM drv_chat_messages WHERE id = MAX(cm.id)) as last_msg_type,
                SUM(CASE WHEN cm.sender_type = 'user' AND cm.is_read = 0 THEN 1 ELSE 0 END) as unread_count
            FROM drv_chat_messages cm
            JOIN drv_candidates c ON c.id = cm.candidate_id
            LEFT JOIN drv_locations l ON l.id = c.location_id";
...
if ($action === 'templates') {
    ...
    $db = getDB();
    $stmt = $db->prepare(
        "SELECT * FROM drv_chat_templates WHERE admin_id = 0 OR admin_id = ? ORDER BY sort_order, id"
    );
...
if ($action === 'save_template' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $db = getDB();
    if ($id > 0) {
        $db->prepare("UPDATE drv_chat_templates SET title = ?, message = ? WHERE id = ? AND (admin_id = ? OR admin_id = 0)")
           ->execute([$title, $message, $id, $admin['id']]);
    } else {
        $db->prepare("INSERT INTO drv_chat_templates (admin_id, title, message) VALUES (?, ?, ?)")
...
if ($action === 'delete_template' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $id = (int)($data['id'] ?? 0);
    if ($id) {
        getDB()->prepare("DELETE FROM drv_chat_templates WHERE id = ? AND admin_id = ?")->execute([$id, $admin['id']]);
    }
    jsonResponse(['ok' => true]);
}
...
if ($action === 'unread') {
    ...
    if ($sender['type'] === 'user') {
        ...
        $stmt = $db->prepare(
            "SELECT COUNT(*) FROM drv_chat_messages WHERE candidate_id = ? AND sender_type = 'admin' AND is_read = 0"
        );
        $stmt->execute([$candidateId]);
        jsonResponse(['count' => (int)$stmt->fetchColumn()]);

    } else {
        // Admin: count total unread across all conversations (from drv_users)
        if ($candidateId) {
            $stmt = $db->prepare(
                "SELECT COUNT(*) FROM drv_chat_messages WHERE candidate_id = ? AND sender_type = 'user' AND is_read = 0"
            );
            $stmt->execute([$candidateId]);
        } else {
            $stmt = $db->prepare(
                "SELECT COUNT(*) FROM drv_chat_messages WHERE sender_type = 'user' AND is_read = 0"
            );
            $stmt->execute();
        }
        jsonResponse(['count' => (int)$stmt->fetchColumn()]);
...
// ═══════════════════════════════════════════════════
// BLAST — Send to multiple drv_candidates
// ═══════════════════════════════════════════════════
...
function triggerChatPush($candidateId, $sender, $preview) {
    ...
    if ($sender['type'] === 'admin') {
        ...
        $stmt = $db->prepare("SELECT u.id FROM drv_users u JOIN drv_candidates c ON c.user_id = u.id WHERE c.id = ?");
        $stmt->execute([$candidateId]);
        $userId = $stmt->fetchColumn();
        if (!$userId) return;

        $subs = $db->prepare("SELECT * FROM drv_push_subscriptions WHERE user_id = ?");
        $subs->execute([$userId]);
    } else {
