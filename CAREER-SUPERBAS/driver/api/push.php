 ...
 *   GET  ?action=unread_count              → count unread notifications
 *   POST ?action=mark_read                 → mark notification(s) as read
 *   GET  ?action=list                      → list notifications for user
 */
...
if ($action === 'unsubscribe' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $endpoint = $data['endpoint'] ?? '';
    if ($endpoint) {
        getDB()->prepare("DELETE FROM push_subscriptions WHERE user_id = ? AND endpoint = ?")->execute([$user['id'], $endpoint]);
    }
...
if ($action === 'send' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmtNotif = $db->prepare(
        "INSERT INTO notifications (user_id, type, title, message, created_by) VALUES (?, 'korlap_note', ?, ?, ?)"
    );
    ...
    $subs = $db->prepare("SELECT * FROM push_subscriptions WHERE user_id IN ($placeholders)");
...
if ($action === 'latest') {
    ...
    $db = getDB();
    $stmt = $db->prepare(
        "SELECT id, title, message, created_at FROM notifications WHERE user_id = ? AND is_read = 0 ORDER BY id DESC LIMIT 1"
    );
    $stmt->execute([$userId]);
    $n = $stmt->fetch();
...
if ($action === 'mark_read' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($data['all'])) {
        ...
        $db->prepare("UPDATE notifications SET is_read = 1 WHERE id = ? AND user_id = ?")->execute([$data['id'], $user['id']]);
    }
    jsonResponse(['ok' => true]);
}
