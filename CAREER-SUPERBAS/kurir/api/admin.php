<?php
/**
 * BAS Kurir — Admin API v3.3 (Schema-Verified)
 * 
 * Known DB schema (from debug 2026-04-09):
 *   krr_candidates: id, given_id, candidate_id, user_id, name, nik, whatsapp, email, address,
 *               provinsi, kabupaten, kecamatan, kelurahan,
 *               referensi, tempat_lahir, tanggal_lahir, pendidikan_terakhir, pernah_kerja_spx,
 *               surat_sehat, paklaring, armada_type, sim_type, sim_number, location_id, status,
 ...
 *   krr_users:  id, username, password, plain_password, name, email, phone, google_id,
 *               picture, is_blacklisted, created_at, last_login
 *   krr_locations:  id, name, address, maps_link
 */
...
if ($method === 'GET') {
    ...
    try { $db->exec("ALTER TABLE krr_candidates ADD COLUMN sim_number VARCHAR(20) DEFAULT NULL AFTER sim_type"); } catch (PDOException $e) {}

    // ── Return krr_locations ──────────────────────────────────────────────
    ...
    if (isset($_GET['audit'])) {
        ...
        try {
            $stmt = $db->prepare('SELECT admin_name, action, old_value, new_value, notes, created_at FROM krr_audit_logs WHERE candidate_id = ? ORDER BY created_at DESC');
            $stmt->execute([$cid]);
        ...
        try {
            $stmt = $db->prepare("
                SELECT c.id, c.given_id, c.candidate_id, c.user_id,
                       ...
                       c.armada_type, c.sim_type, c.sim_number, c.location_id,
                ...
                LEFT JOIN krr_users     u ON c.user_id = u.id
                WHERE c.id = ?
            ");
            ...
            // krr_documents
            try {
                $ds = $db->prepare('SELECT id, doc_type, file_path, original_name, file_size, uploaded_at FROM krr_documents WHERE candidate_id = ?');
    ...
    // All editable fields on krr_candidates table
    $allowed = [
        ...
        'armada_type','sim_type','sim_number','location_id',
    ...
    try {
        ...
        $old = $db->prepare('SELECT * FROM krr_candidates WHERE id = ?');
        $old->execute([$candidateId]);
        $oldRow = $old->fetch();

        $stmt = $db->prepare('UPDATE krr_candidates SET ' . implode(', ', $sets) . ' WHERE id = ?');
        $stmt->execute($vals);
        ...
        try {
            ...
            $log = $db->prepare('INSERT INTO krr_audit_logs (candidate_id, admin_name, action, old_value, new_value) VALUES (?,?,?,?,?)');
            $log->execute([$candidateId, $admin['name'], 'update', json_encode($oldRow ? array_intersect_key($oldRow, $updates) : []), json_encode($changed)]);
        } catch (PDOException $e) {} // krr_audit_logs might not exist
    ...
    if ($action === 'blacklist') {
        ...
        try {
            $stmt = $db->prepare('INSERT INTO krr_blacklists (nik, name, reason, created_by) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE reason=VALUES(reason), updated_at=NOW()');
    ...
    if ($action === 'create_korlap') {
        ...
        try {
            ...
            $stmt = $db->prepare('INSERT INTO krr_admins (username, password, name, role, location_id) VALUES (?,?,?,?,?)');
    ...
    if ($action === 'reset_password') {
        ...
        try {
            $c = $db->prepare('SELECT user_id FROM krr_candidates WHERE id = ?');
            ...
            $stmt    = $db->prepare('UPDATE krr_users SET password = ?, plain_password = ? WHERE id = ?');
    ...
    if ($action === 'delete_korlap') {
        ...
        try {
            $db->prepare('DELETE FROM krr_admins WHERE id = ? AND role != ?')->execute([$kid, 'owner']);
            jsonResponse(['success' => true]);
        } catch (PDOException $e) {
            jsonResponse(['error' => $e->getMessage()], 500);
        }
    ...
    if ($action === 'list_korlaps') {
        try {
            $stmt = $db->query("SELECT id, username, name, role, location_id, created_at FROM krr_admins WHERE role != 'owner' ORDER BY id");
    ...
    if ($action === 'list_blacklists') {
        try {
            $stmt = $db->query('SELECT id, nik, name, reason, created_by, created_at FROM krr_blacklists ORDER BY created_at DESC');
    ...
    if ($action === 'delete_blacklist') {
        ...
        try {
            $db->prepare('DELETE FROM krr_blacklists WHERE id = ?')->execute([$bid]);
            jsonResponse(['success' => true]);
        } catch (PDOException $e) {
            jsonResponse(['error' => $e->getMessage()], 500);
        }
    ...
    if ($action === 'create_location' || $action === 'update_location') {
        ...
        try {
            if ($action === 'create_location') {
                ...
                $stmt = $db->prepare('UPDATE krr_locations SET name=?, address=?, maps_link=? WHERE id=?');
    ...
    if ($action === 'delete_location') {
        ...
        if (!$lid) jsonResponse(['error' => 'ID required'], 400);
        try {
            $db->prepare('DELETE FROM krr_locations WHERE id = ?')->execute([$lid]);
            jsonResponse(['success' => true]);
        } catch (PDOException $e) {
            jsonResponse(['error' => $e->getMessage()], 500);
        }
    ...
    $db = getDB();
    try {
        $db->prepare('UPDATE krr_candidates SET is_deleted = 1 WHERE id = ?')->execute([$id]);
        jsonResponse(['success' => true]);
    } catch (PDOException $e) {
        jsonResponse(['error' => $e->getMessage()], 500);
    }
