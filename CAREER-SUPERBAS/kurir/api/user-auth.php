 ...
 * POST ?action=login            — Login (username OR NIK + password, checks krr_admins then users)
...
switch ($action) {
    ...
    case 'register':
        ...
        $stmt = $db->prepare('SELECT reason FROM krr_blacklists WHERE nik = ?');
        $stmt->execute([$nik]);
        if ($reason = $stmt->fetchColumn()) {
            jsonResponse(['error' => 'Pendaftaran Ditolak. NIK ini telah diblacklist. ' . ($reason ? 'Alasan: ' . $reason : '')], 403);
        }

        $stmt = $db->prepare('SELECT id FROM krr_users WHERE nik = ?');
        $stmt->execute([$nik]);
        if ($stmt->fetch()) { jsonResponse(['error' => 'NIK sudah terdaftar'], 400); }

        $stmt = $db->prepare('SELECT id FROM krr_users WHERE username = ?');
        $stmt->execute([$username]);
        if ($stmt->fetch()) { jsonResponse(['error' => 'Username sudah digunakan'], 400); }

        $stmt = $db->prepare('SELECT id FROM krr_admins WHERE username = ?');
        $stmt->execute([$username]);
        if ($stmt->fetch()) { jsonResponse(['error' => 'Username sudah digunakan'], 400); }

        $stmt = $db->prepare('SELECT id FROM krr_users WHERE email = ?');
        $stmt->execute([$email]);
        if ($stmt->fetch()) { jsonResponse(['error' => 'Email sudah terdaftar'], 400); }

        $hashedPassword = password_hash($password, PASSWORD_DEFAULT);
        $stmt = $db->prepare('INSERT INTO krr_users (nik, username, password, plain_password, name, email, phone) VALUES (?, ?, ?, ?, ?, ?, ?)');
        $stmt->execute([$nik, $username, $hashedPassword, $password, $name, $email, $phone]);
        $userId = $db->lastInsertId();

        // Auto-create candidate record with address
        $stmt = $db->prepare('INSERT INTO krr_candidates (user_id, nik, name, whatsapp, address, provinsi, kabupaten, kecamatan, kelurahan, location_id, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    ...
    case 'login':
        ...
        // 1. Check krr_admins table (username only)
        ...
        $stmt = $db->prepare('SELECT id, nik, username, password, name, picture, is_deleted FROM krr_users WHERE username = ? OR nik = ? OR email = ?');
        ...
        if ($user) {
            ...
            $stmt = $db->prepare('SELECT id FROM krr_blacklists WHERE nik = ?');
        ...
        if ($user && password_verify($password, $user['password'])) {
            $db->prepare('UPDATE krr_users SET last_login = NOW() WHERE id = ?')->execute([$user['id']]);
    ...
    case 'forgot-password':
        ...
        $stmt = $db->prepare('SELECT id, username FROM krr_users WHERE nik = ? AND email = ?');
        ...
        $db->prepare('UPDATE krr_users SET password = ?, plain_password = ? WHERE id = ?')->execute([$hashedPassword, $newPassword, $user['id']]);
    ...
    case 'google':
        ...
        $stmt = $db->prepare('SELECT id, name, picture FROM krr_users WHERE google_id = ?');
        $stmt->execute([$googleId]);
        $existing = $stmt->fetch();

        if ($existing) {
            $db->prepare('UPDATE krr_users SET name = ?, email = ?, picture = ?, last_login = NOW() WHERE id = ?')
        ...
        if ($emailUser) {
            $db->prepare('UPDATE krr_users SET google_id = ?, picture = ?, last_login = NOW() WHERE id = ?')
    ...
    case 'google-complete':
        ...
        $hashedPassword = password_hash($password, PASSWORD_DEFAULT);
        $stmt = $db->prepare('INSERT INTO krr_users (nik, username, password, plain_password, name, email, phone, google_id, picture) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
        ...
        $stmt = $db->prepare('INSERT INTO krr_candidates (user_id, nik, name, whatsapp, location_id, status) VALUES (?, ?, ?, ?, ?, ?)');
