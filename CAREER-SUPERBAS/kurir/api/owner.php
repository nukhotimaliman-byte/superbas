...
switch ($action) {
    ...
    case 'analytics':
        // Total krr_candidates
        $total = $db->query('SELECT COUNT(*) AS cnt FROM krr_candidates')->fetch()['cnt'];

        // By status
        $stmt = $db->query('SELECT status, COUNT(*) AS cnt FROM krr_candidates GROUP BY status');
        $byStatus = [];
        ...
        // By armada type
        $stmt = $db->query('SELECT armada_type, COUNT(*) AS cnt FROM krr_candidates GROUP BY armada_type');
        $byArmada = [];
        while ($row = $stmt->fetch()) {
        ...
        $stmt = $db->query("SELECT DATE(created_at) AS date, COUNT(*) AS cnt FROM krr_candidates WHERE created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY) GROUP BY DATE(created_at) ORDER BY date");
