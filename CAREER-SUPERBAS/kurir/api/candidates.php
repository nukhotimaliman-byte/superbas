<?php
/**
 * BAS Recruitment — krr_candidates API
 * POST /api/krr_candidates.php              — Register new candidate / Update profile / Schedule interview
 * GET  /api/krr_candidates.php?whatsapp=xxx — Status tracking lookup
 * GET  /api/krr_candidates.php?user_id=xxx  — Get candidate by user_id (dashboard)
 */
...
if ($method === 'POST') {
    ...
    if ($action === 'register') {
        ...
        $nik = null;
        if ($user_id) {
            $stmt = $db->prepare('SELECT nik FROM krr_users WHERE id = ?');
        ...
        if ($user_id) {
            $stmt = $db->prepare('SELECT id FROM krr_candidates WHERE user_id = ?');
            $stmt->execute([$user_id]);
        ...
        if ($existingCandidate) {
            ...
            $stmt = $db->prepare('UPDATE krr_candidates SET nik = ?, name = ?, whatsapp = ?, address = ?, provinsi = ?, kabupaten = ?, kecamatan = ?, kelurahan = ?, armada_type = ?, sim_type = ?, location_id = ?, signature_data = ? WHERE id = ?');
            ...
            $stmt = $db->prepare('SELECT id FROM krr_candidates WHERE whatsapp = ? AND whatsapp != ""');
            $stmt->execute([$whatsapp]);
            if ($stmt->fetch()) {
                jsonResponse(['error' => 'Nomor WhatsApp sudah terdaftar.'], 409);
            }

            $stmt = $db->prepare('INSERT INTO krr_candidates (user_id, nik, name, whatsapp, address, provinsi, kabupaten, kecamatan, kelurahan, armada_type, sim_type, location_id, status, signature_data) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    ...
    if (in_array($action, ['request_track','check_track','update_location','get_active_tracks','get_location_history','cancel_track'])) {
        $db = getDB();
        // Auto-create columns on krr_candidates
        try { $db->exec("ALTER TABLE krr_candidates ADD COLUMN track_requested TINYINT(1) DEFAULT 0"); } catch (Exception $e) {}
        try { $db->exec("ALTER TABLE krr_candidates ADD COLUMN last_latitude DOUBLE DEFAULT NULL"); } catch (Exception $e) {}
        try { $db->exec("ALTER TABLE krr_candidates ADD COLUMN last_longitude DOUBLE DEFAULT NULL"); } catch (Exception $e) {}
        try { $db->exec("ALTER TABLE krr_candidates ADD COLUMN last_accuracy FLOAT DEFAULT NULL"); } catch (Exception $e) {}
        try { $db->exec("ALTER TABLE krr_candidates ADD COLUMN last_location_at DATETIME DEFAULT NULL"); } catch (Exception $e) {}

        // Track requests queue
        $db->exec("CREATE TABLE IF NOT EXISTS krr_track_requests (
    ...
    if ($action === 'request_track') {
        ...
        $stmt = $db->prepare('SELECT id FROM krr_track_requests WHERE candidate_id = ? AND status = "pending" LIMIT 1');
        ...
        $stmt = $db->prepare('UPDATE krr_candidates SET track_requested = 1 WHERE id = ?');
        $stmt->execute([$candidate_id]);

        // Add to queue
        $stmt = $db->prepare('INSERT INTO krr_track_requests (candidate_id, candidate_name, candidate_nik, status) VALUES (?, ?, ?, "pending")');
    ...
    if ($action === 'cancel_track') {
        ...
        $stmt = $db->prepare('SELECT candidate_id FROM krr_track_requests WHERE id = ?');
        $stmt->execute([$track_id]);
        $row = $stmt->fetch();
        if ($row) {
            $db->prepare('UPDATE krr_candidates SET track_requested = 0 WHERE id = ?')->execute([$row['candidate_id']]);
        }

        $stmt = $db->prepare('UPDATE krr_track_requests SET status = "cancelled" WHERE id = ? AND status = "pending"');
        $stmt->execute([$track_id]);
    ...
    if ($action === 'check_track') {
        ...
        try {
            $stmt = $db->prepare('SELECT id, track_requested FROM krr_candidates WHERE user_id = ? AND track_requested = 1 LIMIT 1');
            $stmt->execute([$user_id]);
            $row = $stmt->fetch();
    ...
    if ($action === 'update_location') {
        ...
        // Get candidate
        $stmt = $db->prepare('SELECT id FROM krr_candidates WHERE user_id = ? LIMIT 1');
        $stmt->execute([$user_id]);
        $cand = $stmt->fetch();

        // Update candidate latest location + clear flag
        $stmt = $db->prepare('
            UPDATE krr_candidates 
    ...
    if ($action === 'schedule') {
        ...
        $db = getDB();
        try {
            $stmt = $db->prepare('UPDATE krr_candidates SET jadwal_interview = ?, interview_location = ? WHERE id = ?');
    ...
    if ($action === 'update_photo') {
        ...
        $db = getDB();
        $stmt = $db->prepare('UPDATE krr_candidates SET photo_data = ? WHERE id = ?');
    ...
    if ($action === 'submit_pemberkasan') {
        ...
        $sim_number          = trim($data['sim_number'] ?? '');
        $signature           = $data['signature_data'] ?? null;
        ...
        try {
            $db = getDB();
            $stmt = $db->prepare('SELECT status, signature_data FROM krr_candidates WHERE id = ?');
            $stmt->execute([$candidate_id]);
            $existing = $stmt->fetch();
            ...
            $stmt = $db->prepare("
                ...
                SET tempat_lahir = ?, tanggal_lahir = ?, address = ?,
                    ...
                    whatsapp = ?,
                    sim_type = ?, sim_number = ?, armada_type = ?,
    ...
    if ($user_id) {
        $stmt = $db->prepare('
            SELECT c.id, c.given_id, c.candidate_id, c.nik, c.name, c.whatsapp, c.address,
                   ...
                   c.armada_type, c.sim_type, c.sim_number, c.status, c.test_drive_date, c.test_drive_time,
                   c.jadwal_interview,
            ...
            FROM krr_candidates c
            LEFT JOIN krr_locations l ON c.location_id = l.id
            WHERE c.user_id = ?
            LIMIT 1
        ');
        $stmt->execute([$user_id]);
        ...
        // Get krr_documents
        $stmt = $db->prepare('SELECT id, doc_type, file_path, original_name, file_size, uploaded_at FROM krr_documents WHERE candidate_id = ?');
        $stmt->execute([$candidate['id']]);
        $docs = $stmt->fetchAll();

        // Get user info
        $stmt = $db->prepare('SELECT name, email, picture FROM krr_users WHERE id = ?');
        $stmt->execute([$user_id]);
        $user = $stmt->fetch();

        // Get all krr_locations for list display
        $stmt = $db->query('SELECT name, address, maps_link FROM krr_locations ORDER BY name ASC');
    ...
    if ($nik) {
        ...
        $stmt = $db->prepare('SELECT doc_type, uploaded_at FROM krr_documents WHERE candidate_id = ?');
        $stmt->execute([$candidate['id']]);
        $docs = $stmt->fetchAll();
