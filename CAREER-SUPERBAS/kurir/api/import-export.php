<?php
/**
 * BAS Kurir — Import/Export API (Owner only)
 * GET  ?action=export       — Export krr_candidates to JSON (for client-side Excel)
 * POST ?action=import       — Import krr_candidates from JSON (parsed client-side)
 */
...
switch ($action) {
    ...
    case 'export':
        ...
        // Get krr_documents per candidate
        $docStmt = $db->query("SELECT candidate_id, doc_type, original_name FROM krr_documents ORDER BY candidate_id");
        $docs = [];
    ...
    case 'import':
        ...
        // Preload krr_locations
        $locStmt = $db->query("SELECT id, name FROM krr_locations");
        ...
        foreach ($rows as $i => $row) {
            ...
            $simNumber  = trim($row['nomor_sim'] ?? $row['Nomor SIM'] ?? '') ?: null;
            $armada     = trim($row['armada'] ?? $row['Armada'] ?? 'CDD');
            if (!in_array($armada, ['CDD', 'Wingbox', 'Bigmama', 'Big Mama'])) $armada = 'CDD';
            ...
            try {
                ...
                $existing = null;
                if ($nik) {
                    $check = $db->prepare('SELECT id FROM krr_candidates WHERE nik = ? LIMIT 1');
                    $check->execute([$nik]);
                ...
                if ($existing) {
                    ...
                    $updates[] = 'sim_number = ?';     $params[] = $simNumber;
                    ...
                    $params[] = $existing['id'];
                    $db->prepare('UPDATE krr_candidates SET ' . implode(', ', $updates) . ' WHERE id = ?')->execute($params);
                    $updated++;
                } else {
                    // INSERT new candidate
                    $stmt = $db->prepare("
                        INSERT INTO krr_candidates 
                        (given_id, name, nik, whatsapp, email, sim_type, sim_number, armada_type, location_id, status,
                         tempat_lahir, tanggal_lahir, address, pendidikan_terakhir, pernah_kerja_spx,
                         surat_sehat, paklaring, jadwal_interview)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    ");
                    $stmt->execute([
                        $givenId, $name, $nik, $wa, $email, $simType, $simNumber, $armada, $locationId, $status,
