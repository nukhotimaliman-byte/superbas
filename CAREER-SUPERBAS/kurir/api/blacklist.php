...
if ($method === 'GET') {
    ...
    $stmt = $db->query('
        ...
        FROM krr_blacklists b
        LEFT JOIN krr_admins a ON b.created_by = a.id
        LEFT JOIN krr_candidates c ON c.nik = b.nik
...
if ($method === 'POST') {
    ...
    try {
        $stmt = $db->prepare('INSERT INTO krr_blacklists (nik, reason, created_by) VALUES (?, ?, ?)');
...
if ($method === 'DELETE') {
    ...
    $stmt = $db->prepare('DELETE FROM krr_blacklists WHERE id = ?');
    $stmt->execute([$id]);
