 ...
 *   GET  ?action=unread_count              → count unread krr_notifications
 *   POST ?action=mark_read                 → mark notification(s) as read
 *   GET  ?action=list                      → list krr_notifications for user
 */
...
if ($action === 'subscribe' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $db->prepare("DELETE FROM krr_push_subscriptions WHERE endpoint = ?")->execute([$endpoint]);

    $stmt = $db->prepare("INSERT INTO krr_push_subscriptions (user_id, endpoint, p256dh, auth_key) VALUES (?, ?, ?, ?)");
...
if ($action === 'unsubscribe' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $endpoint = $data['endpoint'] ?? '';
    if ($endpoint) {
        getDB()->prepare("DELETE FROM krr_push_subscriptions WHERE user_id = ? AND endpoint = ?")->execute([$user['id'], $endpoint]);
    }
...
if ($action === 'send' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmtNotif = $db->prepare(
        "INSERT INTO krr_notifications (user_id, type, title, message, created_by) VALUES (?, 'korlap_note', ?, ?, ?)"
    );
    ...
    $subs = $db->prepare("SELECT * FROM krr_push_subscriptions WHERE user_id IN ($placeholders)");
...
if ($action === 'latest') {
    ...
    $db = getDB();
    $stmt = $db->prepare(
        "SELECT id, title, message, created_at FROM krr_notifications WHERE user_id = ? AND is_read = 0 ORDER BY id DESC LIMIT 1"
    );
    $stmt->execute([$userId]);
    $n = $stmt->fetch();
...
// ══════════════════════════════════════════════════════
// User: Count unread krr_notifications
// ══════════════════════════════════════════════════════
if ($action === 'unread_count') {
    $user = requireUser();
    $db = getDB();
    $stmt = $db->prepare("SELECT COUNT(*) FROM krr_notifications WHERE user_id = ? AND is_read = 0");
    $stmt->execute([$user['id']]);
    jsonResponse(['count' => (int)$stmt->fetchColumn()]);
}

// ══════════════════════════════════════════════════════
// User: List krr_notifications
// ══════════════════════════════════════════════════════
if ($action === 'list') {
    $user = requireUser();
    $db = getDB();
    $stmt = $db->prepare(
        "SELECT id, type, title, message, is_read, created_at 
         FROM krr_notifications WHERE user_id = ? ORDER BY id DESC LIMIT 50"
    );
    $stmt->execute([$user['id']]);
...
if ($action === 'mark_read' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($data['all'])) {
        $db->prepare("UPDATE krr_notifications SET is_read = 1 WHERE user_id = ?")->execute([$user['id']]);
    } elseif (!empty($data['id'])) {
        $db->prepare("UPDATE krr_notifications SET is_read = 1 WHERE id = ? AND user_id = ?")->execute([$data['id'], $user['id']]);
    }
    jsonResponse(['ok' => true]);
}
