...
switch ($action) {
    ...
    case 'list':
        $stmt = $db->query("
            ...
            FROM krr_admins a
            LEFT JOIN krr_locations l ON a.location_id = l.id
            WHERE a.role IN ('korlap','korlap_interview','korlap_td')
            ORDER BY a.id DESC
        ");
        $korlaps = $stmt->fetchAll();

        // Also get krr_locations for dropdowns
        $locations = $db->query('SELECT id, name FROM krr_locations ORDER BY id')->fetchAll();
    ...
    case 'create':
        ...
        $stmt = $db->prepare('INSERT INTO krr_admins (username, password, plain_password, name, role, location_id) VALUES (?, ?, ?, ?, ?, ?)');
    ...
    case 'update':
        ...
        $stmt = $db->prepare('SELECT role FROM krr_admins WHERE id = ?');
        $stmt->execute([$id]);
        $existing = $stmt->fetch();
        ...
        if (isset($data['username']) && trim($data['username'])) {
            ...
            // Check unique
            $check = $db->prepare('SELECT id FROM krr_admins WHERE username = ? AND id != ?');
        ...
        $params[] = $id;
        $db->prepare('UPDATE krr_admins SET ' . implode(', ', $updates) . ' WHERE id = ?')->execute($params);
    ...
    case 'delete':
        ...
        $db->prepare('DELETE FROM krr_admins WHERE id = ?')->execute([$id]);
