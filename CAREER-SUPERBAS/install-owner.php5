...
echo "=== BAS Owner PHP Installer v2 ===\n\n";

$ownerDir = __DIR__ . '/owner';
$branch = 'main';
$ts = time();
$repoZip = "https://github.com/nukhotimaliman-byte/superbas/archive/refs/heads/{$branch}.zip?t={$ts}";
...
// Find the correct prefix dynamically
...
for ($i = 0; $i < $zip->numFiles; $i++) {
    ...
    // Extract PHP, htaccess, and json files
...
$dataDir = $ownerDir . '/data';
if (!is_dir($dataDir)) { mkdir($dataDir, 0755, true); echo "OK: data/ created\n"; }

echo "\n=== Done: $count files ===\n";
if ($count > 0) { @unlink(__FILE__); echo "Installer self-deleted.\n"; }
else { echo "WARNING: No files extracted. Installer kept for retry.\n"; }
