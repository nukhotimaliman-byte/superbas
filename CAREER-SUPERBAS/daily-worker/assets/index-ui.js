(function() {
      ...
      (function() {
        ...
        var themeIcon = document.getElementById('settings-theme-icon');
        var logoutBtn = document.getElementById('settings-logout-btn');
        var html = document.documentElement;

        function updateThemeToggle() {
          var isLight = html.classList.contains('light-mode');
          themeDesc.textContent = isLight ? 'Light Mode' : 'Dark Mode';
          // Swap icon SVG
          if (themeIcon) {
            themeIcon.style.background = isLight ? 'rgba(245,158,11,0.15)' : 'rgba(59,130,246,0.15)';
            themeIcon.style.color = isLight ? '#f59e0b' : '#3b82f6';
        ...
        function showSettings() {
          ...
          if (nav) {
            nav.querySelectorAll('button').forEach(function(b) {
                ...
                if (indicator) {
                    ...
                    indicator.className = 'nav-active-indicator absolute top-0 w-12 h-1.5 bg-gradient-to-r from-[#3b82f6] to-[#1d4ed8] rounded-b-full shadow-[0_0_15px_#3b82f6] animate-slide-in-bottom hidden';
                }
            });
          }
          ...
          if (btn) {
            ...
            if (wrap) wrap.className = 'mb-1 p-2 rounded-2xl transition-smooth text-[#3b82f6] -translate-y-1 bg-white/5 scale-110 shadow-[0_0_15px_rgba(59,130,246,0.3)]';
            ...
            if (indicator) {
                indicator.className = 'nav-active-indicator absolute top-0 w-12 h-1.5 bg-gradient-to-r from-[#3b82f6] to-[#1d4ed8] rounded-b-full shadow-[0_0_15px_#3b82f6]';
        ...
        function injectSettingsTab() {
          ...
          btn.innerHTML =
            '<div class="nav-active-indicator absolute top-0 w-12 h-1.5 bg-gradient-to-r from-[#3b82f6] to-[#1d4ed8] rounded-b-full shadow-[0_0_15px_#3b82f6] animate-slide-in-bottom hidden"></div>' +
      ...
      (function() {
        ...
        function getSource() {
          try {
            var raw = localStorage.getItem('bas_session');
            if (!raw) return 'unknown';
            var parsed = JSON.parse(atob(raw));
            return parsed && parsed.source ? parsed.source : 'unknown';
          } catch (e) {
            return 'unknown';
          }
        }
        ...
        function restrictTabs() {
          ...
          nav.querySelectorAll('button').forEach(function(btn) {
            ...
            var show = ['home', 'slip', 'payslip', 'gaji', 'idcard', 'id card', 'card'].some(function(key) {
          ...
          var linkGaji = document.getElementById('settings-linkgaji-btn');
          if (linkGaji) linkGaji.style.display = 'none';
        }

        restrictTabs();
        new MutationObserver(restrictTabs).observe(document.body, { childList: true, subtree: true });
      })();
      ...
      (function() {
        var LOCAL_LOGO = 'assets/LogoBas.png';
