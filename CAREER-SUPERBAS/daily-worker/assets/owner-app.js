...
window.onerror = function(msg, src, line, col, err) {
  ...
  if (el) {
    el.innerHTML = '<div style="text-align:left;padding:20px;max-width:600px;margin:auto">'
      + '<h2 style="color:#3b82f6;margin-bottom:10px">⚠ Dashboard Error</h2>'
      ...
      + '<button onclick="location.reload()" style="margin-top:16px;padding:8px 20px;background:#3b82f6;color:#fff;border:none;border-radius:10px;cursor:pointer;font-weight:bold">↻ Reload</button>'
      + '</div>';
  }
};
window.addEventListener('unhandledrejection', function(e) {
  ...
  if (el) {
    el.innerHTML = '<div style="text-align:left;padding:20px;max-width:600px;margin:auto">'
      + '<h2 style="color:#3b82f6;margin-bottom:10px">⚠ Async Error</h2>'
...
// ═══════════════════════════════════════════════════════════════
const DEFAULT_API = "https://script.google.com/macros/s/AKfycbx52a6N0oZN0jzbbsv_tL0um3XwmgK6fVikQLZyyq_JODNT6fhMhjwY9Eop6BAyNxQ0/exec";
let API = localStorage.getItem('bas_api_url') || DEFAULT_API;
let employees=[], attMap={}, payMap={}, charts={}, curTab='overview';
let employeeSummaryCache = null;
let employeeViewState = { q:'', pos:'', st:'', stn:'', sortKey:'name', sortDir:1, limit:100 };
...
// ═══════════════════════════════════════════════════════════════
const C = {
  pk:'rgba(59,130,246,1)', pkB:'rgba(59,130,246,0.12)',
  bl:'rgba(29,78,216,1)', blB:'rgba(29,78,216,0.12)',
  ...
  multi:['rgba(59,130,246,1)','rgba(29,78,216,1)','rgba(76,201,240,1)','rgba(0,245,212,1)','rgba(255,158,0,1)','rgba(157,78,221,1)','rgba(239,68,68,1)','rgba(99,102,241,1)']
};
...
function parseHealthResponse(text){
  ...
  if(json && json.status === 'ONLINE'){
    return { ok:true, mode:'json', version:json.version || '?', raw, json };
  }
  const lower = raw.toLowerCase();
  if(lower.includes('superbas api is running') || lower.includes('api is running') || lower === 'online'){
    return { ok:true, mode:'text', version:'legacy-text', raw, json:null };
  }
  return {
    ok:false,
    mode: json ? 'json' : 'text',
    ...
    raw,
    json,
    message: (json && json.error) ? json.error : (raw || 'Empty response')
  };
}

function inspectGasAction(action, payload){
  const isObj = payload && typeof payload === 'object' && !Array.isArray(payload);
  const error = isObj && payload.error ? String(payload.error) : '';
  const lower = error.toLowerCase();

  if(error){
    if(lower.includes('action tidak dikenal')) return { ok:false, level:'fail', message:error };
    if(lower.includes("sheet 'data'")) return { ok:false, level:'fail', message:'Sheet Data/DataKaryawan tidak ditemukan atau tidak terhubung.' };
    if(lower.includes("sheet 'gaji'")) return { ok:false, level:'fail', message:'Sheet Gaji/SlipGaji tidak ditemukan atau kosong.' };
    return { ok:false, level:'warn', message:error };
  }

  if(action === 'getAllEmployees'){
    if(Array.isArray(payload)) return { ok:true, level:'ok', message:'Karyawan termuat: ' + payload.length };
    return { ok:false, level:'fail', message:'Respons getAllEmployees bukan array.' };
  }

  if(action === 'getPayslips'){
    if(Array.isArray(payload)) return { ok:true, level:'ok', message:'Payslip termuat: ' + payload.length };
    return { ok:false, level:'fail', message:'Respons getPayslips bukan array.' };
  }

  if(action === 'getSystemMessage'){
    if(payload === null) return { ok:true, level:'ok', message:'System message kosong, tapi action tersedia.' };
    if(isObj) return { ok:true, level:'ok', message:'System message tersedia.' };
    return { ok:false, level:'warn', message:'Respons getSystemMessage tidak dikenali.' };
  }

  if(action === 'diagnose' || action === 'discoverSheets'){
    if(isObj && Array.isArray(payload.sheets)) return { ok:true, level:'ok', message:'Sheet ditemukan: ' + payload.sheets.length };
    return { ok:false, level:'warn', message:'Action tersedia tapi tidak mengembalikan daftar sheet.' };
  }

  if(action === 'loginOwner'){
    if(isObj && payload.isOwner === true) return { ok:true, level:'ok', message:'Owner login tervalidasi.' };
    if(isObj && Object.keys(payload).length === 0) return { ok:false, level:'warn', message:'Action loginOwner merespons kosong; verifikasi akun owner manual diperlukan.' };
    return { ok:false, level:'warn', message:'Respons loginOwner belum bisa diverifikasi.' };
  }

  return { ok:true, level:'ok', message:'Action tersedia.' };
}

async function probeGasDeployment(url){
  const report = {
    url,
    health: { ok:false, message:'Belum dicek' },
    checks: [],
    criticalIssues: [],
    warnings: []
  };
  ...
  try {
    const resp = await fetchWithTimeout(url, { method:'GET', headers:{ 'Accept':'application/json, text/plain;q=0.9' } }, 15000);
    const text = await resp.text();
    report.health = parseHealthResponse(text);
    report.health.httpStatus = resp.status;
    if(!resp.ok || !report.health.ok){
      report.criticalIssues.push('Health check gagal: ' + (report.health.message || ('HTTP ' + resp.status)));
      return report;
    }
  } catch(e) {
    report.health = { ok:false, message:e.name === 'AbortError' ? 'Koneksi timeout (15s).' : (e.message || String(e)) };
    report.criticalIssues.push('Health check gagal: ' + report.health.message);
    return report;
  }

  const actions = [
    { action:'getAllEmployees', params:{}, critical:true },
    { action:'getPayslips', params:{ opsId:'OPS123' }, critical:true },
    { action:'getSystemMessage', params:{}, critical:false },
    { action:'diagnose', params:{}, critical:false },
    { action:'discoverSheets', params:{}, critical:false },
    { action:'loginOwner', params:{ opsId:'OPS999', nik:'999' }, critical:false }
  ];
  ...
  for(const test of actions){
    ...
    try {
      const resp = await fetchWithTimeout(url, {
        ...
        body: JSON.stringify({ action:test.action, ...test.params })
      }, 20000);
      const text = await resp.text();
      payload = parseJsonSafe(text);
      if(payload === null && text){
        payload = { error:'Respons bukan JSON: ' + text.substring(0, 140) };
      }
      const summary = inspectGasAction(test.action, payload);
      const check = { action:test.action, critical:test.critical, payload, ...summary };
      report.checks.push(check);
      if(!summary.ok){
        const target = test.critical ? report.criticalIssues : report.warnings;
        target.push(test.action + ': ' + summary.message);
      }
    } catch(e) {
      const msg = e.name === 'AbortError' ? 'Timeout saat memanggil action.' : (e.message || String(e));
      report.checks.push({ action:test.action, critical:test.critical, payload, ok:false, level:'fail', message:msg });
      const target = test.critical ? report.criticalIssues : report.warnings;
      target.push(test.action + ': ' + msg);
    }
  }
...
function renderGasProbeReport(report){
  const critical = report.criticalIssues || [];
  const warnings = report.warnings || [];
  const items = (report.checks || []).map(function(check){
    const color = check.level === 'ok' ? 'text-accent-green' : (check.level === 'warn' ? 'text-accent-orange' : 'text-red-400');
    const icon = check.level === 'ok' ? '✅' : (check.level === 'warn' ? '⚠️' : '❌');
    return '<tr class="border-b border-white/5">'
      + '<td class="py-2 px-2 text-white font-bold">' + esc(check.action) + (check.critical ? ' <span class="chip chip-cyan text-[8px]">critical</span>' : '') + '</td>'
      + '<td class="py-2 px-2 ' + color + '">' + icon + ' ' + esc(check.message || '-') + '</td>'
      + '</tr>';
  }).join('');

  return ''
    + '<div class="space-y-3">'
    +   '<div class="p-4 rounded-xl ' + (critical.length ? 'bg-red-500/10 border border-red-500/30' : 'bg-accent-green/10 border border-accent-green/30') + '">'
    +     '<p class="' + (critical.length ? 'text-red-400' : 'text-accent-green') + ' font-bold text-sm">'
    +       (critical.length ? '❌ Deployment GAS belum kompatibel penuh' : '✅ Deployment GAS lolos pengecekan kritis')
    +     '</p>'
    +     '<p class="text-slate-400 mt-1">Health: <strong class="text-white">' + esc(report.health.mode === 'text' ? 'OK (legacy text)' : 'OK') + '</strong>' + (report.health.version ? ' | Version: <strong class="text-white">' + esc(report.health.version) + '</strong>' : '') + '</p>'
    +   '</div>'
    +   (critical.length ? '<div class="p-3 rounded-xl bg-white/3 border border-red-500/20"><p class="text-red-400 font-bold mb-2">Masalah kritis</p><div class="space-y-1 text-xs text-slate-300">' + critical.map(function(msg){ return '<p>• ' + esc(msg) + '</p>'; }).join('') + '</div></div>' : '')
    +   (warnings.length ? '<div class="p-3 rounded-xl bg-white/3 border border-accent-orange/20"><p class="text-accent-orange font-bold mb-2">Peringatan</p><div class="space-y-1 text-xs text-slate-300">' + warnings.map(function(msg){ return '<p>• ' + esc(msg) + '</p>'; }).join('') + '</div></div>' : '')
    +   '<div class="p-3 rounded-xl bg-white/3 border border-white/5">'
    +     '<p class="font-bold text-white mb-2">Ringkasan action</p>'
    +     '<div class="overflow-x-auto"><table class="w-full text-xs"><tbody>' + items + '</tbody></table></div>'
    +   '</div>'
    + '</div>';
}
...
async function healthCheck(){
  try{
    const r=await fetchWithTimeout(API,{method:'GET',headers:{'Accept':'application/json, text/plain;q=0.9'}},15000);
    if(!r.ok) throw new Error('HTTP '+r.status);
    const text=await r.text();
    ...
    if(API !== DEFAULT_API){
      ...
      try{
        const r2=await fetchWithTimeout(API,{method:'GET',headers:{'Accept':'application/json, text/plain;q=0.9'}},15000);
        if(r2.ok){
...
function normalizeFields(obj, fieldMap) {
  ...
  for (const [standard, aliases] of Object.entries(fieldMap)) {
    ...
    const found = normalizedAliases.map(alias => normalizedMap.get(alias)).find(Boolean);
...
function normalizeEmployeeRecord(record) {
  const normalized = normalizeFields(record || {}, EMP_FIELDS);
  if(!normalized.opsId) normalized.opsId = normalized.nik || '';
  if(!normalized.name) normalized.name = normalized.opsId || '-';
  if(!normalized.bankName && normalized.bank) normalized.bankName = normalized.bank;
  if(!normalized.bankAccount && normalized.rekening) normalized.bankAccount = normalized.rekening;
  if(!normalized.phone && normalized.wa) normalized.phone = normalized.wa;
  if(!normalized.joinDate && normalized.join) normalized.joinDate = normalized.join;
  return normalized;
}

function normalizeAttendanceRecord(record) {
  const normalized = normalizeFields(record || {}, ATT_FIELDS);
  if(!normalized.opsId && normalized.nik) normalized.opsId = normalized.nik;
  if(!normalized.name) normalized.name = normalized.opsId || '-';
  return normalized;
}

function normalizePayslipRecord(record) {
  const normalized = normalizeFields(record || {}, PAY_FIELDS);
  if(!normalized.opsId && normalized.nik) normalized.opsId = normalized.nik;
  if(!normalized.name) normalized.name = normalized.opsId || '-';
  if(typeof normalized.totalDibayarkan === 'string') normalized.totalDibayarkan = parseFloat(normalized.totalDibayarkan.replace(/[^\d.-]/g,'')) || 0;
  if(!normalized.totalDibayarkan && normalized.gaji) normalized.totalDibayarkan = typeof normalized.gaji === 'number' ? normalized.gaji : parseFloat(String(normalized.gaji).replace(/[^\d.-]/g,'')) || 0;
  if(!normalized.period && normalized.periode) normalized.period = normalized.periode;
  return normalized;
}
...
async function loadAll(){
  const allowFallback = arguments.length ? !!arguments[0] : true;
  ...
  const tryFallbackToDefault = async reason => {
    if(!allowFallback || API === DEFAULT_API) return false;
    console.warn('[BAS] Fallback ke DEFAULT_API karena:', reason);
    localStorage.removeItem('bas_api_url');
    API = DEFAULT_API;
    employees=[]; attMap={}; payMap={};
    resetEmployeeSummaryCache();
    dbStatus = { connected: false, error: null, emptySheetsWarning: false };
    setMsg('URL API lama tidak kompatibel. Mencoba URL default...');
    await loadAll(false);
    return true;
  };
  ...
  if(ed===null){
    if(await tryFallbackToDefault('getAllEmployees null response')) return;
    dbStatus.connected=false;
  ...
  if(ed && ed.error && !Array.isArray(ed)){
    if(await tryFallbackToDefault('getAllEmployees error: ' + ed.error)) return;
  ...
  // Set employees
  employees = allEmployees.map(normalizeEmployeeRecord);
  resetEmployeeSummaryCache();
    ...
    if(attArr.length === 0 && payArr.length === 0) {
      ...
      for(let i = 0; i < employees.length; i += batchSize) {
        ...
        const proms = batch.map(async e => {
          ...
          const normA = (Array.isArray(a)?a:[]).map(normalizeAttendanceRecord);
          const normP = (Array.isArray(p)?p:[]).map(normalizePayslipRecord);
...
function resetEmployeeSummaryCache(){
  employeeSummaryCache = null;
}

function getEmployeeSummaryMap(){
  if(employeeSummaryCache) return employeeSummaryCache;
  const summary = {};
  employees.forEach(e => {
    const att = attMap[e.opsId] || [];
    const stationCounts = {};
    for(let i = 0; i < att.length; i++){
      const station = String(att[i].station || '-').toUpperCase();
      stationCounts[station] = (stationCounts[station] || 0) + 1;
    }
    let mainStation = '-';
    let mainCount = 0;
    for(const key in stationCounts){
      if(stationCounts[key] > mainCount){
        mainCount = stationCounts[key];
        mainStation = key;
      }
    }
    summary[e.opsId] = { attCount: att.length, mainStation };
  });
  employeeSummaryCache = summary;
  return summary;
}

function getFilteredEmployeesData(){
  const q = (employeeViewState.q || '').trim().toLowerCase();
  const pos = employeeViewState.pos || '';
  const st = employeeViewState.st || '';
  const stn = (employeeViewState.stn || '').toUpperCase();
  const summaryMap = getEmployeeSummaryMap();

  let list = employees.filter(e => {
    const summary = summaryMap[e.opsId] || { attCount:0, mainStation:'-' };
    const matchQ = !q || [e.name, e.opsId, e.position, e.bankName, e.bank, e.status, summary.mainStation].some(v => String(v || '').toLowerCase().includes(q));
    const matchPos = !pos || (e.position || '') === pos;
    const matchSt = !st || (e.status || '') === st;
    const matchStn = !stn || String(summary.mainStation || '').toUpperCase() === stn;
    return matchQ && matchPos && matchSt && matchStn;
  });

  const sortKey = employeeViewState.sortKey || 'name';
  const dir = employeeViewState.sortDir || 1;
  list = list.slice().sort((a,b) => {
    const sumA = summaryMap[a.opsId] || { attCount:0, mainStation:'-' };
    const sumB = summaryMap[b.opsId] || { attCount:0, mainStation:'-' };
    let av = '';
    let bv = '';
    if(sortKey === 'attCount'){
      av = sumA.attCount;
      bv = sumB.attCount;
    } else if(sortKey === 'mainStation'){
      av = sumA.mainStation;
      bv = sumB.mainStation;
    } else {
      av = a[sortKey] || '';
    ...
    return String(av).localeCompare(String(bv), 'id', { sensitivity:'base', numeric:true }) * dir;
  });
...
function renderEmployeeRows(){
  const tbody = document.getElementById('empTb');
  const countEl = document.getElementById('empCount');
  const moreWrap = document.getElementById('empMoreWrap');
  if(!tbody) return;

  const summaryMap = getEmployeeSummaryMap();
  const filtered = getFilteredEmployeesData();
  const limit = Math.min(employeeViewState.limit || 100, filtered.length);
  const visible = filtered.slice(0, limit);

  tbody.innerHTML = visible.map(e => {
    const summary = summaryMap[e.opsId] || { attCount:0, mainStation:'-' };
    return `<tr class="trow border-b border-white/[0.02]">
      ...
      <td class="px-4 py-3 hidden lg:table-cell"><span class="chip chip-cyan text-[9px]">${esc(summary.mainStation||'-')}</span></td>
      <td class="px-4 py-3 text-right font-bold text-white">${summary.attCount}</td>
    </tr>`;
  }).join('');

  if(countEl) countEl.textContent = limit + ' / ' + filtered.length;
  if(moreWrap){
    moreWrap.innerHTML = filtered.length > limit
      ? `<button onclick="loadMoreEmployees()" class="px-4 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-bold text-white transition-all">Muat ${Math.min(100, filtered.length - limit)} lagi</button>`
      : (filtered.length ? `<span class="text-[10px] text-slate-500">Semua ${filtered.length} baris sudah ditampilkan.</span>` : `<span class="text-[10px] text-slate-500">Tidak ada data yang cocok dengan filter.</span>`);
  }
}

function sortEmployees(key){
  if(employeeViewState.sortKey === key){
    employeeViewState.sortDir = employeeViewState.sortDir * -1;
  } else {
    employeeViewState.sortKey = key;
    employeeViewState.sortDir = key === 'attCount' ? -1 : 1;
  }
  renderEmployeeRows();
}

function loadMoreEmployees(){
  employeeViewState.limit += 100;
  renderEmployeeRows();
}

function sampleEmployeeCounts(values, maxPoints = 240){
  ...
  for(let i = 0; i < maxPoints; i++){
    const rawIndex = Math.min(values.length - 1, Math.floor(i * step));
    sampled.push({ index: rawIndex, value: values[rawIndex] });
  }
  return sampled;
}
...
function rOverview(){
  ...
  setTimeout(()=>{
    ...
    mkChart('cRiskDist',{type:'bar',data:{labels:Object.keys(riskBuckets),datasets:[{label:'Employees',data:Object.values(riskBuckets),backgroundColor:['rgba(0,245,212,0.5)','rgba(76,201,240,0.5)','rgba(255,158,0,0.5)','rgba(59,130,246,0.5)','rgba(239,68,68,0.5)'],borderColor:['rgba(0,245,212,1)','rgba(76,201,240,1)','rgba(255,158,0,1)','rgba(59,130,246,1)','rgba(239,68,68,1)'],borderWidth:1,borderRadius:8}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,ticks:{stepSize:1,font:{size:8}}},x:{ticks:{font:{size:9,weight:'600'}}}}}});
...
function rEmployees(){
  ...
  employeeViewState.limit = Math.max(employeeViewState.limit || 100, 100);
  ...
  el.innerHTML=`
    ...
    <div class="glass rounded-2xl overflow-hidden fade-up" style="animation-delay:80ms">
      <div class="p-4 space-y-3 border-b border-white/5">
        <div class="relative"><svg class="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg><input id="empQ" type="text" placeholder="Cari nama, OPS ID, posisi, bank..." class="search-input" value="${esc(employeeViewState.q||'')}" oninput="debouncedFilterEmp()"></div>
        <div class="flex flex-wrap gap-2">
          <select id="empPos" class="filter-select" onchange="filterEmp()"><option value="">Semua Posisi</option>${positions.map(p=>`<option ${employeeViewState.pos===p?'selected':''}>${esc(p)}</option>`).join('')}</select>
          <select id="empSt" class="filter-select" onchange="filterEmp()"><option value="">Semua Status</option>${statuses.map(s=>`<option ${employeeViewState.st===s?'selected':''}>${esc(s)}</option>`).join('')}</select>
          <select id="empStation" class="filter-select" onchange="filterEmp()"><option value="">Semua Station</option>${stations.map(s=>`<option ${employeeViewState.stn===s?'selected':''}>${esc(s)}</option>`).join('')}</select>
        </div>
        <div class="flex items-center gap-2 text-[10px] text-slate-500 font-semibold"><span id="empCount">0</span> hasil — render bertahap 100 baris untuk menjaga browser tetap responsif</div>
      </div>
      <div class="table-wrap overflow-x-auto">
        <table class="w-full text-xs">
          <thead><tr class="border-b border-white/5 text-left">
            <th class="px-4 py-3 font-bold text-slate-500 uppercase tracking-wider sort-btn cursor-pointer" onclick="sortEmployees('name')">Nama</th>
            <th class="px-4 py-3 font-bold text-slate-500 uppercase tracking-wider sort-btn cursor-pointer" onclick="sortEmployees('opsId')">OPS ID</th>
            <th class="px-4 py-3 font-bold text-slate-500 uppercase tracking-wider sort-btn hidden sm:table-cell cursor-pointer" onclick="sortEmployees('position')">Posisi</th>
            ...
            <th class="px-4 py-3 font-bold text-slate-500 uppercase tracking-wider hidden lg:table-cell cursor-pointer" onclick="sortEmployees('mainStation')">Station Utama</th>
            <th class="px-4 py-3 font-bold text-slate-500 uppercase tracking-wider text-right sort-btn cursor-pointer" onclick="sortEmployees('attCount')">Presensi</th>
          </tr></thead>
          <tbody id="empTb"></tbody>
        </table>
      </div>
      <div id="empMoreWrap" class="p-4 border-t border-white/5 flex items-center justify-center"></div>
    </div>
  ...
  setTimeout(()=>{
    ...
    const chartWrap = document.getElementById('cCluster')?.parentElement;
    if(employees.length > 600){
      if(chartWrap){
        chartWrap.innerHTML = '<div class="h-full flex items-center justify-center text-center text-slate-500 text-xs px-6">Analitik clustering disembunyikan untuk dataset besar agar tab Karyawan tetap responsif. Gunakan tab Overview untuk ringkasan.</div>';
      }
      return;
    }
    const sampled = sampleEmployeeCounts(counts, 220);
    const sampledCounts = sampled.map(item => item.value);
    if(sampledCounts.length>2){
      const k=Math.min(3,sampledCounts.length);
      const res=ML.kMeans(sampledCounts,k);
      const shouldScore = sampledCounts.length <= 160;
      const silScore=shouldScore ? DL.silhouetteScore(sampledCounts,res.assignments,res.centroids) : null;
      ...
      const ds=res.clusters.map((cl,i)=>({
        label:labels[i]||'C'+(i+1),
        data:(cl||[]).map(it=>({x:sampled[it.idx]?.index ?? it.idx,y:Array.isArray(it.data)?it.data[0]:it.data})),
        backgroundColor:colors[i%colors.length],
        pointRadius:sampledCounts.length > 120 ? 3 : 5,
        pointHoverRadius:6
      }));
      const titleText = shouldScore
        ? 'Silhouette Score: ' + silScore.toFixed(3) + ' | WCSS: ' + res.wcss.toFixed(1)
        : 'Approximate clustering (' + sampledCounts.length + ' sample) | WCSS: ' + res.wcss.toFixed(1);
      mkChart('cCluster',{type:'scatter',data:{datasets:ds},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'bottom',labels:{usePointStyle:true,padding:10,font:{size:9,weight:'600'}}},title:{display:true,text:titleText,font:{size:9},color:'rgba(148,163,184,0.6)'}},scales:{x:{title:{display:true,text:'Index Karyawan',font:{size:9}},ticks:{font:{size:8}}},y:{title:{display:true,text:'Jumlah Presensi',font:{size:9}},beginAtZero:true,ticks:{font:{size:8}}}}}});
    }
  },80);
}

function filterEmp(){
  employeeViewState.q = (document.getElementById('empQ')?.value||'').toLowerCase();
  employeeViewState.pos = document.getElementById('empPos')?.value||'';
  employeeViewState.st = document.getElementById('empSt')?.value||'';
  employeeViewState.stn = document.getElementById('empStation')?.value||'';
  employeeViewState.limit = 100;
  renderEmployeeRows();
}
      ...
      <div class="card-inner">
        <div class="top-bar">
          ...
          <div class="side-bars" style="display:flex;gap:6px">
            <div class="bar bar-blue" style="width:6px;height:24px;background:#1d4ed8;opacity:0.8;border-radius:9999px"></div>
            <div class="bar bar-pink" style="width:6px;height:16px;background:#3b82f6;opacity:0.8;margin-top:8px;border-radius:9999px"></div>
          </div>
        </div>
...
async function downloadAllCards() {
  ...
  for (const { opsId, canvas } of items) {
    ...
    link.download = 'KARTU_IDENTITAS_' + opsId + '.jpg';
...
async function downloadAsZip() {
  ...
  for (const { opsId, canvas } of items) {
    ...
    folder.file('KARTU_IDENTITAS_' + opsId + '.jpg', base64, { base64: true });
  }
...
function printAllCards() {
  ...
  w.document.write(`<!DOCTYPE html><html><head><title>Print ID Cards - BAS</title>
    ...
    <style>
      ...
      .bar-blue{box-shadow:0 0 8px rgba(29,78,216,0.6);}
      .bar-pink{box-shadow:0 0 8px rgba(59,130,246,0.6);}
      .qr-section{position:relative;margin-bottom:24px;}
      .qr-wrap{padding:12px;border-radius:24px;background:rgba(10,5,24,0.8);border:1px solid rgba(29,78,216,0.25);box-shadow:0 0 30px rgba(29,78,216,0.15),inset 0 1px 0 rgba(255,255,255,0.05);}
      ...
      .emp-name{font-weight:900;text-transform:uppercase;font-size:15px;color:#ffffff;line-height:1.25;letter-spacing:0.025em;text-shadow:0 2px 8px rgba(0,0,0,0.6),0 0 20px rgba(29,78,216,0.15);padding:0 10px;margin-bottom:8px;overflow-wrap:break-word;}
      .pos-line{display:flex;align-items:center;justify-content:center;gap:8px;opacity:0.9;}
      .pos-bar{height:2px;width:30px;background:linear-gradient(90deg,transparent,#3b82f6,transparent);}
      .pos-text{font-weight:700;text-transform:uppercase;letter-spacing:0.2em;font-size:10px;color:#3b82f6;}
...
async function saveApiUrl(){
  ...
  showToast('Memeriksa kompatibilitas deployment GAS...','info');
  const probe = await probeGasDeployment(val);
  const diagnoseResult = document.getElementById('diagnoseResult');
  if(diagnoseResult) diagnoseResult.innerHTML = renderGasProbeReport(probe);

  if(!probe.health.ok || probe.criticalIssues.length){
    showToast('URL GAS belum kompatibel. Lihat hasil diagnosa di bawah.','error');
    return;
  }
...
async function runDiagnose(){
  ...
  try {
    ...
    let healthData = null;
    try {
      const hc = await fetch(API, {method:'GET', headers:{'Accept':'application/json, text/plain;q=0.9'}});
      if(hc.ok) {
        const healthText = await hc.text();
        healthData = parseHealthResponse(healthText);
        healthOk = !!healthData.ok;
      }
    ...
    log.push('✅ Health check OK: ' + (healthData?.mode === 'text' ? 'legacy text response' : 'JSON ONLINE') + ' v' + (healthData?.version||'?'));
    ...
    // ── STEP 4: Compatibility Probe (non-mutating only) ──
    log.push('🔄 Testing compatibility actions (non-mutating)...');
    const probe = await probeGasDeployment(API);
    results.probe = probe;
    probe.checks.forEach(ch => {
      const prefix = ch.level === 'ok' ? '✅' : (ch.level === 'warn' ? '⚠' : '❌');
      log.push(prefix + ' ' + ch.action + ': ' + ch.message);
    });

    const diagEntry = probe.checks.find(ch => ch.action === 'diagnose' && ch.ok && ch.payload && ch.payload.sheets);
    results.diagnose = diagEntry ? diagEntry.payload : null;
    ...
    const compatibilityHtml = results.probe ? renderGasProbeReport(results.probe) : '';
...
const PHOTO_API = resolvePhotoApiUrl();
const photoMonitorState = {
  page: 1,
  limit: 24,
  searchTimer: null,
  stations: []
  ...
  el.innerHTML = `
    <div class="space-y-4">
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-2 w-full sm:w-auto sm:min-w-[760px]">
          <input type="date" id="pm-date" value="${today}" onchange="applyPhotoFilters(true)" 
          ...
          <select id="pm-station" onchange="applyPhotoFilters(true)" class="bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white focus:ring-2 focus:ring-accent-pink outline-none">
            <option value="">Semua Station</option>
          </select>
          <select id="pm-status" onchange="applyPhotoFilters(true)" class="bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white focus:ring-2 focus:ring-accent-pink outline-none">
            <option value="">Semua Status</option>
            <option value="PENDING">Pending</option>
            <option value="DITERIMA">Diterima</option>
            <option value="DITOLAK">Ditolak</option>
          </select>
          <input type="text" id="pm-search" placeholder="Cari nama/OPS..." oninput="queuePhotoSearch()"
      ...
      <div id="pm-pagination" class="hidden items-center justify-between gap-3 glass rounded-2xl border border-white/5 p-4"></div>
    </div>
    <!-- Photo Modal -->
...
function renderPhotoStationFilter(stations, selectedStation){
  const stationEl = document.getElementById('pm-station');
  if (!stationEl) return;
  const safeStations = Array.isArray(stations) ? stations.filter(Boolean) : [];
  photoMonitorState.stations = safeStations;
  stationEl.innerHTML = [
    '<option value="">Semua Station</option>',
    ...safeStations.map(st => `<option value="${esc(st)}">${esc(st)}</option>`)
  ].join('');
  stationEl.value = selectedStation || '';
}

window.applyPhotoFilters = function(resetPage) {
  if (resetPage !== false) {
    photoMonitorState.page = 1;
  }
  loadPhotoLogs();
};

window.queuePhotoSearch = function() {
  clearTimeout(photoMonitorState.searchTimer);
  photoMonitorState.searchTimer = setTimeout(() => {
    photoMonitorState.page = 1;
    loadPhotoLogs();
  }, 250);
};

window.setPhotoPage = function(page) {
  const nextPage = Number(page) || 1;
  if (nextPage < 1 || nextPage === photoMonitorState.page) return;
  photoMonitorState.page = nextPage;
  loadPhotoLogs();
};
...
async function loadPhotoLogs(){
  ...
  try {
    const page = photoMonitorState.page;
    const url = `${PHOTO_API}?action=all-logs&date=${encodeURIComponent(date)}&station=${encodeURIComponent(station)}&status=${encodeURIComponent(status)}&search=${encodeURIComponent(search)}&page=${page}&limit=${photoMonitorState.limit}`;
    ...
    const total = summary.total || result.total || data.length;
    const masuk = summary.masuk || 0;
    const keluar = summary.keluar || 0;
    const totalKaryawan = summary.karyawan || 0;
    const pending = summary.pending || 0;
    const diterima = summary.diterima || 0;
    const ditolak = summary.ditolak || 0;
    const pages = Math.max(1, Number(result.pages) || 1);
    const currentPage = Math.min(Math.max(1, Number(result.page) || 1), pages);
    photoMonitorState.page = currentPage;
    renderPhotoStationFilter(result.filters && result.filters.stations, result.filters && result.filters.station);
    const statusEl = document.getElementById('pm-status');
    if (statusEl) statusEl.value = (result.filters && result.filters.status) || status;
    ...
    if (statsEl) {
      statsEl.innerHTML = `
        ...
        <div class="glass rounded-2xl p-4 border border-white/5 text-center">
          <p class="text-2xl font-black text-cyan-400">${totalKaryawan}</p>
        ...
        <div class="glass rounded-2xl p-4 border border-white/5 text-center">
          <p class="text-2xl font-black text-emerald-400">${diterima}</p>
          <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Diterima</p>
        </div>
        <div class="glass rounded-2xl p-4 border border-white/5 text-center">
          <p class="text-2xl font-black text-rose-400">${ditolak}</p>
      ...
      return `
        <div class="glass rounded-2xl p-4 border border-white/5 flex flex-col sm:flex-row items-start sm:items-center gap-4 hover:border-accent-pink/30 transition-all">
          <div class="flex items-center gap-3 w-full sm:w-auto">
            ...
            <img src="${esc(d.foto_url)}" loading="lazy" decoding="async" class="w-16 h-16 rounded-xl object-cover cursor-pointer shadow-lg border border-white/10 shrink-0" 
    ...
    if (paginationEl) {
      const start = total === 0 ? 0 : ((currentPage - 1) * (result.limit || photoMonitorState.limit)) + 1;
      const end = Math.min(total, start + data.length - 1);
      paginationEl.classList.remove('hidden');
      paginationEl.classList.add('flex');
      paginationEl.innerHTML = `
        <div>
          <p class="text-sm font-bold text-white">Menampilkan ${start}-${end} dari ${total} data</p>
          <p class="text-[11px] text-slate-400">Halaman ${currentPage} dari ${pages}</p>
        </div>
        <div class="flex items-center gap-2 ml-auto">
          <button onclick="setPhotoPage(${currentPage - 1})" ${currentPage <= 1 ? 'disabled' : ''} class="px-4 py-2 rounded-xl text-sm font-bold border border-white/10 bg-white/5 text-white disabled:opacity-40 disabled:cursor-not-allowed">Sebelumnya</button>
          <button onclick="setPhotoPage(${currentPage + 1})" ${currentPage >= pages ? 'disabled' : ''} class="px-4 py-2 rounded-xl text-sm font-bold border border-white/10 bg-white/5 text-white disabled:opacity-40 disabled:cursor-not-allowed">Berikutnya</button>
        </div>
      `;
    }
    refreshPhotoBulkUI();
