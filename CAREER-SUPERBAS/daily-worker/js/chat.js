 ...
 */
const ChatEngine = (() => {
    const API = '/Daily Worker/api/chat.php';
    let _config = {};
    let _pollXHR = null;
    let _lastMsgId = 0;
    let _polling = false;
    ...
    function renderBubble(msg) {
        ...
        switch (msg.message_type) {
            case 'image':
                content = `
                    <div class="chat-image-wrap" onclick="ChatEngine.openImage(this)">
                        <img src="/Daily Worker/${msg.file_path}" alt="${escHtml(msg.file_name)}" loading="lazy">
                    </div>
            ...
            case 'file':
                content = `
                    <a href="/Daily Worker/${msg.file_path}" target="_blank" class="chat-file-card" download="${escHtml(msg.file_name)}">
