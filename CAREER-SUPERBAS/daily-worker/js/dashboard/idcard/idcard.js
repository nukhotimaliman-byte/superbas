 ...
 * Usage: BASIdCard.render(containerId, userData, onDone)
 *        BASIdCard.renderToCanvas(canvas, userData, theme, onDone)
 */
...
var BASIdCard = (function() {
  ...
  function render(containerId, userData, onDone) {
    _containerId = containerId;
    _userData = userData;
    var container = document.getElementById(containerId);
    if (!container) { if (onDone) onDone(null); return; }
    ...
    drawCard(_theme, onDone);
  }

  function drawCard(theme, onDone) {
    _canvas = document.getElementById('basIdCardCanvas');
    if (!_canvas || !_userData) { if (onDone) onDone(null); return; }

    var W = 638, H = 1012;
    _canvas.width = W; _canvas.height = H;
    var ctx = _canvas.getContext('2d');
    var t = THEMES[theme || _theme];

    _fillCard(ctx, W, H, t, theme || _theme, _userData, function() {
      if (onDone) onDone(_canvas);
    });
  }

  // ── Direct canvas render for batch (no DOM needed) ──
  function renderToCanvas(canvas, userData, theme, onDone) {
    if (!canvas || !userData) { if (onDone) onDone(null); return; }
    theme = theme || 'dark';
    ...
    _fillCard(ctx, W, H, t, theme, userData, function() {
      if (onDone) onDone(canvas);
    });
  }

  // ── Core drawing (shared by render + renderToCanvas) ──
  function _fillCard(ctx, W, H, t, theme, userData, onDone) {
    // Background gradient
      ...
      try {
        var doDrawLogo = function(img) {
          ...
          if (theme === 'light') {
            ...
            tctx.drawImage(img, 0, 0, logoS, logoS);
            ...
            ctx.globalAlpha = t.logoAlpha;
            ctx.drawImage(img, logoX, logoY, logoS, logoS);
          }
          ctx.globalAlpha = 1;
          drawTexts(ctx, W, H, t, userData);
          if (onDone) onDone();
        };
  ...
  function drawTexts(ctx, W, H, t, userData) {
    // Title
    ctx.fillStyle = t.sub;
    ...
    ctx.textAlign = 'center';
    ctx.fillText(userData.ops_id || '', W / 2, qrY + qrSize + 65);

    // Name
    ctx.fillStyle = t.text;
    ctx.font = '700 32px Inter, sans-serif';
    ctx.fillText((userData.nama || '').toUpperCase(), W / 2, qrY + qrSize + 110);
  }
