...
const MLEngine = (() => {
    ...
    function extractFeatures(c) {
        return {
            ...
            sim_quality:      SIM_SCORES[c.pendidikan_terakhir] || 0.3,
            ...
            pemberkasan:      ['Sudah Pemberkasan','Menunggu Test Drive','Jadwal Test Drive','Proses Follow Up','Sudah Follow Up','Undang WI','Lulus'].includes(c.status) ? 1.0 : 0.0,
    ...
    function assessRisk(candidate) {
        ...
        if (['Sudah Pemberkasan','Menunggu Test Drive','Proses Follow Up','Sudah Follow Up'].includes(candidate.status) && !candidate.test_drive_date) {
    ...
    function calculateReadiness(c) {
        ...
        score += c.status === 'Proses Follow Up' ? 20 : 0;
        score += c.status === 'Sudah Follow Up' ? 30 : 0;
        score += c.status === 'Undang WI' ? 35 : 0;
    ...
    function generateInsights(analytics, candidates) {
        ...
        // Document completion
        const lowDoc = (candidates || []).filter(c => (c.doc_count || 0) < 3 && !['Lulus','Tidak Lulus','Undang WI'].includes(c.status));
        if (lowDoc.length > 0) {
