...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && ($_GET['action'] ?? '') === 'options') {
    try {
        $stmt = $pdo->query("SELECT * FROM dw_dropdown_options ORDER BY category, sort_order, id");
...
// Check role
$stmt = $pdo->prepare("SELECT role FROM dw_admins WHERE id = ?");
...
switch ($action) {
    ...
    case 'add':
        ...
        if ($order === 0) {
            $stmt = $pdo->prepare("SELECT COALESCE(MAX(sort_order),0)+1 FROM dw_dropdown_options WHERE category = ?");
            $stmt->execute([$cat]);
            $order = $stmt->fetchColumn();
        }

        $stmt = $pdo->prepare("INSERT INTO dw_dropdown_options (category, label, value, color, sort_order) VALUES (?,?,?,?,?)");
    ...
    case 'update':
        ...
        $stmt = $pdo->prepare("UPDATE dw_dropdown_options SET label=?, value=?, color=?, sort_order=? WHERE id=?");
    ...
    case 'delete':
        ...
        $stmt = $pdo->prepare("DELETE FROM dw_dropdown_options WHERE id = ?");
        $stmt->execute([$id]);
    ...
    case 'reorder':
        ...
        $stmt = $pdo->prepare("UPDATE dw_dropdown_options SET sort_order = ? WHERE id = ?");
