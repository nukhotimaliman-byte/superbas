<?php
/**
 * BAS Recruitment — dw_locations API (Owner Only for Write)
 * GET  /api/dw_locations.php — Get all dw_locations
 * POST /api/dw_locations.php — Create new location
 * DELETE /api/dw_locations.php — Delete location
 */
...
if ($method === 'GET') {
    // Public/Admin can get dw_locations
...
if ($method === 'PUT') {
    ...
    $stmt = $db->prepare('UPDATE dw_locations SET maps_link = ? WHERE id = ?');
...
if ($method === 'DELETE') {
    ...
    // Check if used by dw_candidates
    $stmt = $db->prepare('SELECT COUNT(*) FROM dw_candidates WHERE location_id = ?');
    $stmt->execute([$id]);
    if ($stmt->fetchColumn() > 0) {
        jsonResponse(['error' => 'Tidak dapat menghapus lokasi karena sedang digunakan oleh kandidat'], 400);
    }

    $stmt = $db->prepare('DELETE FROM dw_locations WHERE id = ?');
    $stmt->execute([$id]);
