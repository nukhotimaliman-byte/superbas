...
// dw_chat_messages table
try {
    $db->exec("CREATE TABLE IF NOT EXISTS dw_chat_messages (
    ...
    echo "✅ dw_chat_messages table ready\n";
} catch (Exception $e) {
    echo "❌ dw_chat_messages: " . $e->getMessage() . "\n";
}

// dw_chat_templates table (custom templates)
try {
    $db->exec("CREATE TABLE IF NOT EXISTS dw_chat_templates (
        id         INT AUTO_INCREMENT PRIMARY KEY,
    ...
    echo "✅ dw_chat_templates table ready\n";

    // Insert defaults if empty
    $count = $db->query("SELECT COUNT(*) FROM dw_chat_templates")->fetchColumn();
    if ($count == 0) {
        $defaults = [
        ...
        $stmt = $db->prepare("INSERT INTO dw_chat_templates (admin_id, title, message, sort_order) VALUES (0, ?, ?, ?)");
    ...
    echo "❌ dw_chat_templates: " . $e->getMessage() . "\n";
}
...
// Add reply columns to dw_chat_messages
try {
    $db->exec("ALTER TABLE dw_chat_messages ADD COLUMN reply_to_id INT DEFAULT NULL AFTER is_read");
...
try {
    $db->exec("ALTER TABLE dw_chat_messages ADD COLUMN reply_preview TEXT DEFAULT NULL AFTER reply_to_id");
