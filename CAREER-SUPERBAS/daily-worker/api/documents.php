 ...
 * POST /api/dw_documents.php — Upload document with watermark
 * GET  /api/dw_documents.php?id=X — Serve document for preview (admin only)
 */
...
if ($method === 'POST') {
    ...
    $db = getDB();
    $stmt = $db->prepare('SELECT id FROM dw_candidates WHERE id = ?');
    ...
    $stmt = $db->prepare('SELECT id FROM dw_documents WHERE candidate_id = ? AND doc_type = ?');
    ...
    // Save or update DB
    if ($existingDoc) {
        $stmt = $db->prepare('UPDATE dw_documents SET file_path = ?, original_name = ?, file_size = ?, uploaded_at = NOW() WHERE id = ?');
        $stmt->execute([$filename, $file['name'], $file['size'], $existingDoc['id']]);
    } else {
        $stmt = $db->prepare('INSERT INTO dw_documents (candidate_id, doc_type, file_path, original_name, file_size) VALUES (?, ?, ?, ?, ?)');
    ...
    $db = getDB();
    $stmt = $db->prepare('SELECT file_path, original_name FROM dw_documents WHERE id = ?');
