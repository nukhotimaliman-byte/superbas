...
(function () {
  ...
  //  CONFIG
  // ═══════════════════════════════════════════
  const API_URL = 'https://super-bas.com/api/absen-foto.php';
  ...
  function togglePanel(show) {
    ...
    if (show) {
      ...
      if (logoWm && !logoWm.getAttribute('src')) {
        logoWm.src = window.__BAS_WHITE_LOGO || 'assets/bas-logo.png';
      }
  ...
  function capturePhoto() {
    ...
    if (!basLogoImg) {
      ...
      basLogoImg.src = window.__BAS_WHITE_LOGO || 'assets/bas-logo.png';
    }

    // Set canvas size (max 640px)
    const maxDim = 640;
  ...
  async function captureAndUpload(action) {
    ...
    try {
      const payload = {
        action: action,
        ops_id: user.opsId || user.ops_id || user.ops || '',
      ...
      if (result.error) {
        ...
        showStatus(`✅ Absen ${tipe} berhasil! ${result.timestamp || ''}`, 'success');
        loadHistory();
      }
    } catch (err) {
  ...
  async function loadHistory() {
    ...
    const opsId = user.opsId || user.ops_id || user.ops || '';
    try {
  ...
  window.showAttendanceDetail = function(log) {
    ...
    document.getElementById('pa-detail-photo').src = log.foto_url || 'assets/bas-logo.png';
  ...
  async function loadStations() {
    ...
    try {
      ...
      const gasApi = localStorage.getItem('bas_api_url') || 'https://script.google.com/macros/s/AKfycbyFnq_vA10udPChk73NAD5gjCS6YiT8867gylbFGHAns1VvspUF_fcWr_JDT7y4fe50/exec';
